...
const GradientWaves = ({
  ...
  const stepCapRef = useRef(Infinity);

  useEffect(() => {
    const container = containerRef.current;
    if (!container) return undefined;

    const isConstrainedDevice = window.matchMedia('(max-width: 768px)').matches
      || Number(navigator.hardwareConcurrency || 8) <= 4
      || Number(navigator.deviceMemory || 8) <= 4;
    stepCapRef.current = isConstrainedDevice ? 44 : Infinity;
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      dpr: Math.min(window.devicePixelRatio || 1, isConstrainedDevice ? 1 : 1.5),
    });
    ...
    const supportsPointerInteraction = mouseInteraction && window.matchMedia('(hover: hover) and (pointer: fine)').matches;
    ...
    const minimumFrameInterval = isConstrainedDevice ? 1000 / 30 : 0;
    let lastRenderedAt = 0;

    const loop = (t) => {
      raf = requestAnimationFrame(loop);
      if (minimumFrameInterval && t - lastRenderedAt < minimumFrameInterval) return;
      lastRenderedAt = t;
    ...
    const tryStart = () => {
      if (!prefersReducedMotion && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
  ...
  useEffect(() => {
    ...
    u.uSteps.value = Math.min(detailToSteps(detail), stepCapRef.current);
